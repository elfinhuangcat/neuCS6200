...
public class SearchEngineLucene {
    ...
    public static void main(String[] args) throws IOException {
		...
		// Search from args[2] : the query file
		...
		while ((line = br.readLine()) != null) {
			try {
				PrintWriter writer = new PrintWriter(("List_Q" + queryNum), "UTF-8");
				IndexSearcher searcher = new IndexSearcher(reader);
				TopScoreDocCollector collector = TopScoreDocCollector.create(3205, true);
				System.out.println("Query " + queryNum + ": " + line);
				...
				for (int i = 0; i < hits.length; ++i) {
					...
					// 		+ " score=" + hits[i].score);
					if (i < 100) {
						writer.println(d.get("path") + " " + hits[i].score);
					}
				}
				queryNum += 1;
				writer.close();
				...
				System.out.println("Error searching " + line + " : "
						+ e.getMessage());
				break;
			}
		}
