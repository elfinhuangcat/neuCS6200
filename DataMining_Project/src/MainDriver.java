...
public class MainDriver {
	...
	public static void runAllClassifiers(String logFilePath, String trainArffPath) {
		...
		try {
			...
			Log.addLogToThisPath("### Logistic Regression: Vary ridge estimator\n", logFilePath);
			for (Integer ridge = -10; ridge < 6; ++ridge) {
				errorRate = DiffClassifier.run(new String[] {logFilePath, trainArffPath, 
						"-logr", "-R", ridge.toString()}); 
				Log.addLogToThisPath("*************[-logr -R " + ridge.toString() +
						"] " + errorRate + " *************\n", logFilePath);
			}
